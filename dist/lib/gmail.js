 ...
 */
export async function exchangeCode(clientId, clientSecret, code, storage) {
    const oauth2Client = new google.auth.OAuth2(clientId, clientSecret, REDIRECT_URI);
    ...
    if (!tokens.refresh_token) {
        throw new Error('OAuth2 token exchange did not return a refresh_token. ' +
 ...
 * storage.
 */
export async function getAuthenticatedClient(storage, config) {
    const tokens = (await storage.get(KEYS.gmailTokens));
    if (!tokens) {
        throw new Error('Gmail is not authenticated. Run the gmailAuth tool first to connect your Google account.');
    }
    const clientId = config.get('GOOGLE_CLIENT_ID');
    const clientSecret = config.get('GOOGLE_CLIENT_SECRET');
    if (!clientId || !clientSecret) {
        throw new Error('Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET in trik configuration.');
    }
    const oauth2Client = new google.auth.OAuth2(clientId, clientSecret, REDIRECT_URI);
    const credentials = {
        access_token: tokens.access_token,
    ...
    if (tokens.expiry_date <= now + 60_000) {
        try {
            ...
            throw new Error('Gmail access token expired and could not be refreshed. Please re-authenticate using the gmailAuth tool.');
        }
    }
 ...
 */
export async function searchEmails(gmail, sender, maxResults = 10) {
    ...
    for (let i = 0; i < messageIds.length; i += CONCURRENCY) {
        ...
        const settled = await Promise.all(batch.map(async (msg) => {
            if (!msg.id)
                return null;
...
export async function getEmailContent(gmail, messageId) {
    ...
    if (!payload) {
        throw new Error(`Gmail message ${messageId} has no payload — message may be deleted or inaccessible.`);
    }
    let html = '';
    let text = '';
 ...
 */
export function extractLinksFromEmail(htmlContent) {
    if (!htmlContent)
        return [];
    ...
    $('a[href]').each((_, el) => {
        ...
        if (skipPatterns.some((p) => p.test(href)))
            return;
