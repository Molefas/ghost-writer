...
export function manageReferences(storage) {
    return tool(async (input) => {
        switch (input.action) {
            case 'add': {
                ...
                const existing = await getAll(storage, KEYS.referenceIndex, KEYS.reference);
                const match = existing.find((r) => r.name.toLowerCase() === input.name.toLowerCase());
                if (match) {
                    // Merge new topics into existing reference
                    const existingLower = match.topics.map((t) => t.toLowerCase());
                    const newTopics = (input.topics ?? []).filter((t) => !existingLower.includes(t.toLowerCase()));
                    if (newTopics.length > 0) {
                        ...
                        try {
                            ...
                            return JSON.stringify({ error: `Failed to merge topics: ${err.message}` });
                        }
                ...
                try {
                    ...
                    return JSON.stringify({ error: `Failed to add reference: ${err.message}` });
                }
                return JSON.stringify({ action: 'added', reference: ref });
            }
            case 'list': {
                const all = await getAll(storage, KEYS.referenceIndex, KEYS.reference);
                let results = all;
                if (input.topics && input.topics.length > 0) {
                    const filterTopics = input.topics.map((t) => t.toLowerCase());
                    results = all.filter((r) => r.topics.some((t) => filterTopics.includes(t.toLowerCase())));
                }
                return JSON.stringify({
                    action: 'listed',
            ...
            case 'delete': {
                ...
                const ref = await getById(storage, KEYS.reference, input.referenceId);
                if (!ref) {
                ...
                try {
                    ...
                    return JSON.stringify({ error: `Failed to delete reference: ${err.message}` });
                }
        ...
        description: "Manage the user's reference library of books and thought leaders. Use 'add' to add a new reference (deduplicates by name, merges topics if exists), 'list' to view all (optionally filtered by topics), 'delete' to remove one, 'search' to find by name/author/topic.",
        schema: z.object({
            action: z
