...
export function searchInspirations(storage) {
    return tool(async (input) => {
        let inspirations = await getAll(storage, KEYS.inspirationIndex, KEYS.inspiration);
        const criteria = [];
        // Filter by query (title + description)
        if (input.query) {
            const q = input.query.toLowerCase();
            inspirations = inspirations.filter((i) => i.title.toLowerCase().includes(q) ||
        ...
        if (input.tags && input.tags.length > 0) {
            ...
            inspirations = inspirations.filter((i) => i.tags.some((t) => tags.has(t.toLowerCase())));
        ...
        if (input.since) {
            ...
            inspirations = inspirations.filter((i) => new Date(i.addedAt).getTime() >= sinceDate);
