...
export function manageVoice(storage) {
    return tool(async (input) => {
        if (input.action === 'read') {
            const content = await storage.get(KEYS.profileVoice) ?? '';
        ...
        name: 'manageVoice',
        description: "Read or update the user's writing voice profile. Use action 'read' to load the current profile, 'update' to save a new one.",
        schema: z.object({
