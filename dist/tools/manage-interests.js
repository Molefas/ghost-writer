...
export function manageInterests(storage) {
    return tool(async (input) => {
        if (input.action === 'read') {
            const content = await storage.get(KEYS.profileInterests) ?? '';
        ...
        description: "Read or update the user's topic interests. Use action 'read' to load current interests, 'update' to save new ones. Interests are used to score article relevance.",
        schema: z.object({
