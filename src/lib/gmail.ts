    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (!tokens.access_token || !tokens.refresh_token) {
          res.writeHead(200, { 'Content-Type': 'text/html' });
          res.end(
            '<html><body><h2>Authentication failed</h2><p>Missing tokens. You can close this tab.</p></body></html>',
          );
          cleanup();
          return;
        }
        ...
        res.writeHead(200, { 'Content-Type': 'text/html' });
        res.end(
          '<html><body><h2>Gmail connected!</h2><p>You can close this tab and return to the conversation.</p></body></html>',
        );
        cleanup();
      } catch {
        res.writeHead(200, { 'Content-Type': 'text/html' });
        res.end(
          '<html><body><h2>Something went wrong</h2><p>You can close this tab and try again.</p></body></html>',
        );
        cleanup();
      }
    });
