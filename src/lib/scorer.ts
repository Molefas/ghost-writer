export function scoreInspiration(
  title: string,
  description: string,
  interestsContent: string,
): number {
  const keywords = extractKeywords(interestsContent);
  if (keywords.length === 0) return 5; // Default score when no interests defined
...
function extractKeywords(interestsContent: string): string[] {
