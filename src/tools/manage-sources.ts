...
export function manageSources(storage: TrikStorageContext) {
  return tool(
    async (input) => {
      switch (input.action) {
        case 'add': {
        ...
        case 'list': {
          const sources = await getAll<Source>(storage, KEYS.sourceIndex, KEYS.source);
    ...
    },
    {
      name: 'manageSources',
      description:
        'Add, list, remove, or update content sources (blogs, articles, newsletters)',
      schema: z.object({
        action: z
