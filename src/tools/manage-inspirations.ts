...
export function manageInspirations(storage: TrikStorageContext) {
  return tool(
    async (input) => {
      switch (input.action) {
        case 'delete': {
        ...
        case 'deleteMany': {
          ...
          for (const id of input.inspirationIds) {
            const insp = await getById<Inspiration>(storage, KEYS.inspiration, id);
            if (!insp) {
    ...
    },
    {
      ...
      description:
        'Delete one or more inspirations. Use delete for a single inspiration, ' +
